...
1. 下载 `origin-agent-0.2.7-windows-x64.zip`，核对随发行提供的 SHA-256，解压到普通本地目录。
...
无需另装 Python、Node、uv 或编译器。默认程序在 `%USERPROFILE%\.origin-agent\app\0.2.7`，研究产物和会话在 `%USERPROFILE%\.origin-agent`。不改动 Origin 安装及许可；默认引擎安装不注册自启，云端用户可另外启用下方的私有隧道登录任务。包没有商业代码签名；哈希证明传输完整性，不能替代发布者签名。
...
```powershell
& "$env:USERPROFILE\.origin-agent\app\0.2.7\server\origin-agent.exe" rollback-install <receipt_id>
```

自检与诊断：

```powershell
& "$env:USERPROFILE\.origin-agent\app\0.2.7\server\origin-agent.exe" doctor --native
```
...
- **其他 MCP Agent**：通用配置在 `.origin-agent/host-configs/0.2.7/generic-mcp.json`。各宿主连接入口和插件格式不同，运行引擎和工作流契约共用，实际使用仍需宿主验收。
